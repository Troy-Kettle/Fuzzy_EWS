...
smaller clinically-annotated dataset instead:
  ...
  • inspired oxygen scored in its RECORDED units: INSPIRED_O2 + INSPIRED_O2_UNITS is
    ...
    build_scored_sample_spreadsheet_annotated.py (see engine_scoring.
...
grid_search_excess_rowlevel_auprc_annotated.py). The whole dataset (829 admissions,
~54k rows) is used — already well under any sampling cap.

Grid: α 0.1→1.0 (×10) × β 0.0→4.5 (×10) × γ 0.1→1.0 (×10) = 1,000 combos.

Outputs → results/new_system/annotated/{patient level, row-level}/grid_search/
"""
...
OUT_DIR     = REPO / "results" / "new_system" / "annotated"
